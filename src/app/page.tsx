...
// ─── Return date picker dialog state ───────────────────────────────────────

interface ReturnDateDialogState {
  show: boolean
...
export default function HomePage() {
  ...
  const refreshContracts = useCallback(() => {
    fetch('/api/contracts').then(r => r.json()).then(data => setContracts(data)).catch(() => {})
  }, [])
  ...
  const handleReturnDateConfirm = async () => {
    ...
    try {
      ...
      const data = await fetch('/api/contracts').then(r => r.json()).catch(() => [])
      setContracts(data)
      const updatedContract = data.find((c: Contract) => c.id === contract.id) || contract
  ...
  const onSigned = async () => {
    if (activeKind === 'pickup' && selectedContract) {
      // After pickup signature, update contract status to checkin_pending
      try {
        await fetch(`/api/contracts/${selectedContract.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ status: 'checkin_pending' }),
        })
      } catch { /* ignore */ }
  ...
  const handleCreateContractComplete = () => {
    ...
    return (
      <>
        <DashboardView
          ...
          onCreateClick={() => setShowCreateContract(true)}
        ...
        <AnimatePresence>
          {showCreateContract && (
            <CreateContractDialog
              ...
              onComplete={handleCreateContractComplete}
            />
          )}
        </AnimatePresence>
        <AnimatePresence>
          {returnDateDialog.show && (
            <ReturnDatePickerDialog
              contract={returnDateDialog.contract!}
...
function DashboardView({
  ...
  onCreateClick, onDeleteContract, deletingId,
}: {
  ...
  onCreateClick: () => void; onDeleteContract: (id: string) => void; deletingId: string | null
}) {
  ...
  const filtered = contracts.filter(c => {
    ...
    return (
      c.reservationNumber.toLowerCase().includes(s) ||
      c.customerName.toLowerCase().includes(s) ||
      c.vehicleModel.toLowerCase().includes(s) ||
      c.vehicleReg.toLowerCase().includes(s)
    )
  })

  const activeContracts = filtered.filter(c => {
    if (isFullyCompleted(c)) return false
    // If contract is checkin_pending, only show if returnDate <= today
    if (c.status === 'checkin_pending' && c.returnDate) {
      const retStr = typeof c.returnDate === 'string' ? c.returnDate : new Date(c.returnDate).toISOString()
      return retStr.startsWith(today) || retStr < today
    }
    return true
  })
    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="relative">
          ...
          <Input placeholder="Search by reservation, customer, vehicle..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 h-12 text-base bg-white border-slate-200" />
        </div>

        {/* Calendar */}
        <CheckinCalendar
          contracts={contracts}
          onSelectContract={onInspect}
        />
...
function PickupSignView({ contract, damages, userRole, onBack, onSigned }: {
  ...
  const handleSubmit = async () => {
    ...
    try { await fetch('/api/contracts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contractId: contract.id, kind: 'pickup', signaturePngBase64, signerName }) }) } catch { /* ignore */ }
...
function ReturnReportView({ contract, damages, userRole, onBack, onSigned }: {
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    const canvas = canvasRef.current; const signaturePngBase64 = canvas ? canvas.toDataURL('image/png') : ''
    try { await fetch('/api/contracts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contractId: contract.id, kind: 'return', signaturePngBase64, signerName }) }) } catch { /* ignore */ }
      ...
      >
        <div className="p-6 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-slate-900">Return Date</h2>
              <p className="text-xs text-slate-500">Set the expected return date before starting pickup</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
            ...
            <p className="mt-0.5">{contract.vehicleModel} ({contract.vehicleReg})</p>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {saving ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Saving...</> : <><ArrowUpRight className="w-4 h-4 mr-2" />Start Pick Up</>}
            </Button>
          </div>
        </div>
      </motion.div>
...
function CreateContractDialog({ onClose, onComplete }: { onClose: () => void; onComplete: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError('')
    if (!form.reservationNumber.trim() || !form.customerName.trim()) {
      setError('Reservation number and customer name are required.')
      return
    }
    setSaving(true)
    try {
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ scale: 0.95, opacity: 0 }}
        onClick={e => e.stopPropagation()}
        className="w-full max-w-lg bg-white rounded-xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
      >
        <div className="p-6 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Create a manual rental contract</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600">
            <X className="w-5 h-5" />
          ...
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <Input value={form.reservationNumber} onChange={(e) => updateField('reservationNumber', e.target.value)} placeholder="e.g. RNT-12345" required />
              </div>
              <div className="space-y-1">
                <Label>Customer Name *</Label>
                <Input value={form.customerName} onChange={(e) => updateField('customerName', e.target.value)} placeholder="e.g. John Smith" required />
              </div>
              <div className="space-y-1">
                <Label>Customer Email</Label>
                <Input type="email" value={form.customerEmail} onChange={(e) => updateField('customerEmail', e.target.value)} placeholder="customer@example.com" />
              </div>
              <div className="space-y-1">
                <Label>Vehicle Model</Label>
                <Input value={form.vehicleModel} onChange={(e) => updateField('vehicleModel', e.target.value)} placeholder="e.g. VW Golf" />
              </div>
              <div className="space-y-1">
                <Label>Vehicle Registration</Label>
                <Input value={form.vehicleReg} onChange={(e) => updateField('vehicleReg', e.target.value)} placeholder="e.g. MAL-1234" />
              </div>
              <div className="space-y-1">
                <Label>Status</Label>
                <div className="flex gap-2">
                  {(['checkout_pending', 'checkin_pending'] as const).map(s => (
                    <button
                      key={s}
                      type="button"
                      onClick={() => updateField('status', s)}
                      className={`flex-1 p-2 rounded-md text-xs font-medium border-2 transition-colors ${
                        form.status === s
                          ? s === 'checkout_pending' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-purple-500 bg-purple-50 text-purple-700'
                          : 'border-slate-200 text-slate-500 hover:border-slate-300'
                      }`}
                    >
                      {s === 'checkout_pending' ? 'Pickup Pending' : 'Check-in Pending'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <Input type="date" value={form.returnDate} onChange={(e) => updateField('returnDate', e.target.value)} min={form.pickupDate} />
              </div>
            </div>
            {error && (
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button type="submit" disabled={saving} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold">
                {saving ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Creating...</> : <><Plus className="w-4 h-4 mr-2" />Create Contract</>}
              </Button>
            </div>
          </div>
        </form>
